...
import '../../../core/constant/strings/searchpage_strings.dart';
import '../../../core/theme/color/color_theme.dart';
import '../../foodDetailPage/view/detail_view.dart';
import '../model/cateory_card_model.dart';
import '../service/search_service.dart';
import '../viewModel/search_cubit.dart';
import '../../../product/widgets/bottom_navbar.dart';
...
class _SearchPageViewState extends State<SearchPageView> {
  ...
  _buildTitle() {
    return // Tarif Ara
        Text(SearchPageStrings().searchTitleText,
            style: TextStyle(
                color: AppColors().red, fontWeight: FontWeight.w400, fontStyle: FontStyle.normal, fontSize: 24.0),
          ...
          child: InkWell(
              onTap: () {
                showDialog(
                    ...
                    builder: (context) {
                      _allFoods.shuffle();
                      var _whellItems = List.generate((_allFoods.length < 10) ? _allFoods.length : 10, (index) {
                        return FortuneItem(
                            child: FittedBox(
                              child: Text(
                                "    " + _allFoods[index].name!,
                                style: TextStyle(color: (index % 2 == 0) ? AppColors().black : AppColors().darkGrey),
                              ),
                            ),
                            ...
                            child: Column(
                              children: [
                                ...
                                Expanded(
                                  flex: 5,
                                  child: Padding(
                                    padding: EdgeInsets.symmetric(vertical: context.paddingLow.bottom),
                                    child: FortuneWheel(
                                      animateFirst: false,
                                  ...
                                  child: Padding(
                                    ...
                                    child: Center(
                                      child: Text(
                                        ...
                                        style: TextStyle(fontSize: context.dynamicWidth(0.05)),
                                      ),
                                    ),
                                  ),
                                ),
                                        ...
                                        ? FittedBox(
                                            child: Row(
                                              children: [
                                                CircleAvatar(
                                                  backgroundImage:
                                                      NetworkImage(_allFoods[_selectedIndex].imageUrls!.first),
                                                ),
                                                SizedBox(
                                      ...
                                      ElevatedButton(
                                            ...
                                            Navigator.push(
                                                ...
                                                MaterialPageRoute(
                                                  builder: (context) => FoodDetailPageView(
                                                    foodModel: _allFoods[_selectedIndex],
                                                  ),
                                                ));
                                          }
                                        },
                                        style: ButtonStyle(
                                          backgroundColor: (_isTurned)
                                              ? MaterialStateProperty.all<Color>(AppColors().redremains)
                                              : MaterialStateProperty.all<Color>(
                                                  AppColors().redremains.withOpacity(0.6)),
                                        ),
                                        child: Text(
                                          "Go to Recepie",
  ...
  _buildSearchBar(BuildContext context) {
    return // Rectangle 284
      ...
      decoration: const BoxDecoration(
          borderRadius: BorderRadius.all(Radius.circular(16)),
          boxShadow: [BoxShadow(color: Color(0x40000000), offset: Offset(0, 4), blurRadius: 4, spreadRadius: 0)],
          color: Color(0xfff0f0f0)),
      child: Padding(
        padding: context.paddingLow
            .copyWith(bottom: 0, left: context.paddingLow.left * 2, right: context.paddingLow.right * 2),
        child: TextField(
            ...
            child: InkWell(
              ...
              child: Container(
                decoration: BoxDecoration(
                  color: (categories[index].selected) ? AppColors().red : AppColors().red.withOpacity(0.4),
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Padding(
                      ...
                      child: Container(
                        decoration:
                            BoxDecoration(color: AppColors().redremains, borderRadius: context.normalBorderRadius),
                        child: ListTile(
                          ...
                          trailing: IconButton(
                            ...
                            onPressed: () {
                              Navigator.push(context,
                                  MaterialPageRoute(builder: (context) => FoodDetailPageView(foodModel: _item)));
                            },
                          ),
                        ),
                      ),
  ...
  _searchItems(String val) {
    if (val == "null" || val == "") {
      ...
      _matchingList.clear();
      _matchingList
          .addAll(_allFoods.where((element) => element.name!.toLowerCase().contains(val.toLowerCase()) == true));
  ...
  _orderMatchesByCategories(String val) {
    if (_selectedCategories.isNullOrEmpty) {
      ...
      List<String> _selectedCategoriesStrings =
          List.generate(_selectedCategories.length, (index) => _selectedCategories[index].category);
      List<FoodModel> _ref = [];
